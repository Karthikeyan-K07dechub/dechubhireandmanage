...
const sourceHtmlPath = path.join(projectRoot, 'public', 'solutions', 'it', 'index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'deelIt');
...
async function main() {
  ...
  const title = extractSection(/<title>([\s\S]*?)<\/title>/i, html, 'IT Solutions | Deel');
...
export const DEEL_IT_PAGE_TITLE = ${JSON.stringify(decodeHtml(title))};
export const DEEL_IT_HTML_CLASSES = ${JSON.stringify(htmlClasses)};
export const DEEL_IT_ROUTE_SLUG = "/solutions/it";
export const DEEL_IT_STYLESHEET_HREFS = ${JSON.stringify(stylesheetHrefs, null, 2)} as const;
export const DEEL_IT_INLINE_STYLES = ${JSON.stringify(inlineStyles, null, 2)} as const;

export function DeelItContent() {
  return (
    h(Fragment, null,
    ${bodyCode}
  ));
