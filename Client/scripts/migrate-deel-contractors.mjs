...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/solutions/payroll/contractors/index.html");
const outputRoot = path.resolve(projectRoot, "public/solutions/payroll/contractors");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/contractors/", "/solutions/payroll/contractors"],
  ["https://www.deel.com/contractors", "/solutions/payroll/contractors"],
  ["https://www.deel.com/solutions/payroll/contractors/", "/solutions/payroll/contractors"],
  ["https://www.deel.com/solutions/payroll/contractors", "/solutions/payroll/contractors"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `solutions/payroll/contractors/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/solutions/payroll/contractors/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(text) {
  return text.replace(/\/_next\/image\/\?url=([^"'`\s<>&]+(?:%[0-9A-Fa-f]{2}[^"'`\s<>&]*)*)(?:&amp;|&)w=\d+(?:&amp;|&)q=\d+/g, (match, encodedUrl) => {
    try {
      ...
      return `/solutions/payroll/contractors/${localRelativePath}`;
    } catch {
      return match;
    }
  });
...
function removeTrackedDependencies(html) {
  let next = html;

  trackedPatterns.forEach((pattern) => {
    next = next.replace(pattern, (match) => {
      removedDependencies.push(match.trim());
      return "";
    });
  });
...
function extractInlineStyles(html) {
  ...
  const inlineStyles = styleMatches.map((match) => match[1]).filter(Boolean);
  const withoutStyles = html.replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "");

  return { inlineStyles, withoutStyles };
}
...
function extractStylesheetHrefs(html) {
  ...
  next = next.replace(/<link[^>]+rel="stylesheet"[^>]+href="([^"]+)"[^>]*>\s*/gi, (_, href) => {
    hrefs.push(href);
    return "";
  });

  return { hrefs, withoutStylesheets: next };
}
...
async function main() {
  ...
  const seedUrls = extractAssetUrls(sourceHtml);
  await crawlAssets(seedUrls);

  let html = sourceHtml;
  html = replaceMappedUrls(html);
  html = rewriteNextImageOptimizerUrls(html);
  html = rewriteExternalAnchors(html);
  html = removeTrackedDependencies(html);

  const { inlineStyles, withoutStyles } = extractInlineStyles(html);
  const { hrefs, withoutStylesheets } = extractStylesheetHrefs(withoutStyles);
  ...
  await fs.writeFile(outputHtmlPath, normalizeWhitespace(withoutStylesheets));
