...
const sourceHtmlPath = path.join(projectRoot, 'public', 'deel-vs-competitors', 'index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'deelVsCompetitors');
const outputPath = path.join(outputDir, 'generatedPageData.tsx');
const routeSlugFrom = '/deel-vs-competitors';
const routeSlugTo = '/dechub-bridge-vs-competitors';
...
function getFilteredBodyNodes(nodes) {
  ...
  if (
    ...
    && filteredNodes[0].type === 'element'
    && filteredNodes[0].tagName === 'main'
  ) {
    return filteredNodes[0].children;
  }
...
async function main() {
  ...
  const html = sourceHtml
    .replaceAll(`href="${routeSlugFrom}/"`, `href="${routeSlugTo}/"`)
    .replaceAll(`href="${routeSlugFrom}"`, `href="${routeSlugTo}"`);
  const htmlClassMatch = html.match(/<html[^>]*class="([^"]*)"/i);
  const titleMatch = html.match(/<title>([\s\S]*?)<\/title>/i);
  const headStyleMatches = [...html.matchAll(/<style\b[^>]*>([\s\S]*?)<\/style>/gi)];
  const headLinkMatches = [...html.matchAll(/<link[^>]+rel="stylesheet"[^>]+href="([^"]+)"/gi)];
  const bodyMatch = html.match(/<body[^>]*>([\s\S]*?)<\/body>/i);

  if (!bodyMatch) {
    throw new Error('Unable to locate body markup in localized page');
  }
...
export const DEEL_VS_COMPETITORS_PAGE_TITLE = ${JSON.stringify(decodeHtml(titleMatch?.[1] ?? 'Deel vs Competitors'))};
export const DEEL_VS_COMPETITORS_HTML_CLASSES = ${JSON.stringify((htmlClassMatch?.[1] ?? '').trim())};
export const DEEL_VS_COMPETITORS_STYLESHEET_HREFS = ${JSON.stringify(headLinkMatches.map((match) => match[1]), null, 2)} as const;
export const DEEL_VS_COMPETITORS_INLINE_STYLES = ${JSON.stringify(
    headStyleMatches.map((match) => match[1]),
    null,
    2,
  )} as const;

export function DeelVsCompetitorsContent() {
  return h(
    Fragment,
    null,
      ${bodyCode}
