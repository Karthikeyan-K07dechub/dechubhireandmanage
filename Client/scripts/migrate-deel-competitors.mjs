...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/deel-vs-competitors/index.html");
const outputRoot = path.resolve(projectRoot, "public/deel-vs-competitors");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/deel-vs-competitors/", "/deel-vs-competitors"],
  ["https://www.deel.com/deel-vs-competitors", "/deel-vs-competitors"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `deel-vs-competitors/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/deel-vs-competitors/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(html) {
  return html.replace(/\/_next\/image\/\?url=([^"'&]+)([^"'<>]*)/gi, (_, encodedUrl) => {
    try {
      const remoteUrl = normalizeUrl(decodeHtml(decodeURIComponent(encodedUrl)));
      if (!isDownloadableAssetUrl(remoteUrl)) {
        return "#";
      }

      const localRelativePath = localPathForUrl(remoteUrl);
      return `/deel-vs-competitors/${localRelativePath}`;
    } catch {
      return "#";
    }
  });
...
function stripTrackingAndHeadNoise(html) {
  let next = html;
...
async function rewriteDownloadedTextAssets() {
  for (const [, localRelativePath] of assetMap.entries()) {
    const ext = path.extname(localRelativePath).toLowerCase();
    if (!textAssetExtensions.has(ext)) {
      continue;
    }

    const absolutePath = path.join(outputRoot, localRelativePath);
    let content = await fs.readFile(absolutePath, "utf8");
    content = replaceMappedUrls(content);
...
async function collectLocalAssets() {
  ...
  async function walk(relativeDir) {
    ...
    for (const entry of entries) {
      ...
      const childAbsolutePath = path.join(outputRoot, childRelativePath);
      const stats = await fs.stat(childAbsolutePath);
      assets.push({
        localPath: `deel-vs-competitors/${childRelativePath}`,
        bytes: stats.size,
      });
    }
  }
  ...
  await walk("assets");
  return assets.sort((a, b) => a.localPath.localeCompare(b.localPath));
}
...
async function main() {
  ...
  const assetUrls = extractAssetUrls(sourceHtml);

  await crawlAssets(assetUrls);
  await rewriteDownloadedTextAssets();

  let outputHtml = sourceHtml;
  outputHtml = stripTrackingAndHeadNoise(outputHtml);
  outputHtml = replaceMappedUrls(outputHtml);
  outputHtml = rewriteNextImageOptimizerUrls(outputHtml);
  outputHtml = rewriteExternalAnchors(outputHtml);

  await fs.writeFile(outputHtmlPath, outputHtml, "utf8");

  const remainingRemoteUrls = [...new Set(extractAssetUrls(outputHtml))];
  const localAssets = await collectLocalAssets();

  const report = {
    generatedAt: new Date().toISOString(),
    pagePath: "/deel-vs-competitors",
    downloadedAssetCount: downloadedAssets.length,
    downloadedAssets,
    localAssetCount: localAssets.length,
    localAssets,
    removedExternalDependencyPatterns: removedDependencies,
    remainingDownloadableRemoteAssetUrls: remainingRemoteUrls,
  };

  await fs.writeFile(reportPath, JSON.stringify(report, null, 2), "utf8");
  console.log(`Localized ${downloadedAssets.length} assets into ${outputRoot}`);
}
