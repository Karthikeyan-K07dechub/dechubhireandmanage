...
const sourceHtmlPath = path.join(projectRoot, 'public', 'solutions', 'benefits', 'index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'deelBenefits');
...
export const DEEL_BENEFITS_PAGE_TITLE = ${JSON.stringify(decodeHtml(title))};
export const DEEL_BENEFITS_HTML_CLASSES = ${JSON.stringify(htmlClasses)};
export const DEEL_BENEFITS_ROUTE_SLUG = "/solutions/benefits";
export const DEEL_BENEFITS_STYLESHEET_HREFS = ${JSON.stringify(stylesheetHrefs, null, 2)} as const;
export const DEEL_BENEFITS_INLINE_STYLES = ${JSON.stringify(inlineStyles, null, 2)} as const;

export function DeelBenefitsContent() {
  return (
    h(Fragment, null,
    ${bodyCode}
  ));
