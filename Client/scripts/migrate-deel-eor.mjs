...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/solutions/payroll/eor/index.html");
const outputRoot = path.resolve(projectRoot, "public/solutions/payroll/eor");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/solutions/payroll/eor/", "/solutions/payroll/eor"],
  ["https://www.deel.com/solutions/payroll/eor", "/solutions/payroll/eor"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `solutions/payroll/eor/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/solutions/payroll/eor/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(text) {
  return text.replace(/\/_next\/image\/\?url=([^"'`\s<>&]+(?:%[0-9A-Fa-f]{2}[^"'`\s<>&]*)*)(?:&amp;|&)w=\d+(?:&amp;|&)q=\d+/g, (match, encodedUrl) => {
    try {
      ...
      return `/solutions/payroll/eor/${localRelativePath}`;
    } catch {
      return match;
    }
  });
...
function rewriteInternalRoutes(text) {
  let next = text;

  routeMappings.forEach((localRoute, remoteRoute) => {
    const encodedRemoteRoute = remoteRoute.replace(/&/g, "&amp;");
    next = next.replace(new RegExp(escapeRegex(remoteRoute), "g"), localRoute);
    if (encodedRemoteRoute !== remoteRoute) {
      next = next.replace(new RegExp(escapeRegex(encodedRemoteRoute), "g"), localRoute);
    }
  });

  next = next.replace(/href="\/solutions\/payroll\/eor\/"/g, 'href="/solutions/payroll/eor/"');
  next = next.replace(/href="\/solutions\/payroll\/eor"/g, 'href="/solutions/payroll/eor"');
...
async function main() {
  ...
  trackedPatterns.forEach((pattern) => {
    ...
    if (matches) {
      removedDependencies.push(...matches.map((match) => match.slice(0, 160)));
  ...
  html = rewriteInternalRoutes(html);
  ...
  console.log(`Migrated EOR page to ${outputHtmlPath}`);
}
