...
const sourceHtmlPath = path.resolve(projectRoot, '../New_Page/integrations/index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'deelIntegrations');
  ...
  const entries = styleText
    ...
    .map((part) => {
      ...
      if (rawName.startsWith('--')) {
        ...
        propName = /^[$A-Z_][0-9A-Z_$]*$/i.test(camelName) ? camelName : JSON.stringify(camelName);
      }
...
function attrKeyToCode(attrName) {
  return /^[$A-Z_][0-9A-Z_$]*$/i.test(attrName) ? attrName : JSON.stringify(attrName);
}
...
function attrsToCode(rawAttrs) {
  ...
  while ((match = attrRegex.exec(rawAttrs)) !== null) {
    ...
    if (rawValue == null) {
      attrs.push(booleanAttrs.has(normalizedName) ? `${attrKeyToCode(normalizedName)}: true` : `${attrKeyToCode(normalizedName)}: ""`);
      continue;
    }

    attrs.push(`${attrKeyToCode(normalizedName)}: ${JSON.stringify(decodeHtml(rawValue))}`);
  }
...
function extractDocumentParts(html) {
  ...
  if (!bodyMatch) {
    throw new Error('Unable to find <body>...</body> in integrations HTML.');
  }

  const cleanedBodyHtml = bodyMatch[1]
    .replace(/<script\b[\s\S]*?<\/script>/gi, '')
    .replace(/<noscript\b[\s\S]*?<\/noscript>/gi, '');

  return {
    htmlClasses: htmlClassMatch?.[1] ?? '',
    title: decodeHtml(titleMatch?.[1] ?? 'Deel Integrations'),
...
export const DEEL_INTEGRATIONS_PAGE_TITLE = ${JSON.stringify(title)};
export const DEEL_INTEGRATIONS_HTML_CLASSES = ${JSON.stringify(htmlClasses)};
export const DEEL_INTEGRATIONS_STYLESHEET_HREFS = ${JSON.stringify(stylesheetHrefs, null, 2)};
export const DEEL_INTEGRATIONS_INLINE_STYLES = ${JSON.stringify(inlineStyles, null, 2)};

export function DeelIntegrationsContent() {
...
`;
  ...
  console.log(`Generated React page data for integrations at ${outputPath}`);
}
