...
const sourceHtmlPath = path.resolve(projectRoot, '../New_Page/solutions/open-api/index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'deelOpenApi');
...
function extractDocumentParts(html) {
  ...
  if (!bodyMatch) {
    throw new Error('Unable to find <body>...</body> in open-api HTML.');
  }
  ...
  return {
    ...
    title: decodeHtml(titleMatch?.[1] ?? 'Deel Open API'),
...
export const DEEL_OPEN_API_PAGE_TITLE = ${JSON.stringify(title)};
export const DEEL_OPEN_API_HTML_CLASSES = ${JSON.stringify(htmlClasses)};
export const DEEL_OPEN_API_STYLESHEET_HREFS = ${JSON.stringify(stylesheetHrefs, null, 2)};
export const DEEL_OPEN_API_INLINE_STYLES = ${JSON.stringify(inlineStyles, null, 2)};

export function DeelOpenApiContent() {
...
`;
  ...
  console.log(`Generated React page data for open api at ${outputPath}`);
}
