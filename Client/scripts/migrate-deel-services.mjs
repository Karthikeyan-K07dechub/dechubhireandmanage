...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/solutions/services/index.html");
const outputRoot = path.resolve(projectRoot, "public/solutions/services");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/solutions/services/", "/solutions/services"],
  ["https://www.deel.com/solutions/services", "/solutions/services"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `solutions/services/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/solutions/services/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(text) {
  return text.replace(/\/_next\/image\/\?url=([^"'`\s<>&]+(?:%[0-9A-Fa-f]{2}[^"'`\s<>&]*)*)(?:&amp;|&)w=\d+(?:&amp;|&)q=\d+/g, (match, encodedUrl) => {
    try {
      ...
      return `/solutions/services/${localRelativePath}`;
    } catch {
      return match;
    }
  });
...
function rewriteInternalRoutes(text) {
  ...
  next = next.replace(/href="\/solutions\/services\/"/g, 'href="/solutions/services/"');
  next = next.replace(/href="\/solutions\/services"/g, 'href="/solutions/services"');
...
async function main() {
  ...
  console.log(`Migrated services page to ${outputHtmlPath}`);
}
