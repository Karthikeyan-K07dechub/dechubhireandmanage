...
const sourceHtmlPath = path.join(projectRoot, 'public', 'solutions', 'payroll', 'eor', 'index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'deelEor');
...
function extractDocumentParts(html) {
  ...
  if (!bodyMatch) {
    throw new Error('Unable to find <body>...</body> in EOR HTML.');
  }
...
export const DEEL_EOR_PAGE_TITLE = ${JSON.stringify(title)};
export const DEEL_EOR_HTML_CLASSES = ${JSON.stringify(htmlClasses)};
export const DEEL_EOR_STYLESHEET_HREFS = ${JSON.stringify(stylesheetHrefs, null, 2)};
export const DEEL_EOR_INLINE_STYLES = ${JSON.stringify(inlineStyles, null, 2)};

export function DeelEorContent() {
...
`;
  ...
  console.log(`Generated React page data for EOR at ${outputPath}`);
}
