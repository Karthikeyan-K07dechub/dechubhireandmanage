...
const sourceHtmlPath = path.join(projectRoot, 'public', 'solutions', 'payroll', 'index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'payrollSolutions');
const outputPath = path.join(outputDir, 'generatedPageData.tsx');
const routeSlug = '/solutions/payroll';
...
export const PAYROLL_SOLUTIONS_PAGE_TITLE = ${JSON.stringify(decodeHtml(titleMatch?.[1] ?? 'Payroll Solutions'))};
export const PAYROLL_SOLUTIONS_HTML_CLASSES = ${JSON.stringify((htmlClassMatch?.[1] ?? '').trim())};
export const PAYROLL_SOLUTIONS_ROUTE_SLUG = ${JSON.stringify(routeSlug)};
export const PAYROLL_SOLUTIONS_STYLESHEET_HREFS = ${JSON.stringify(headLinkMatches.map((match) => match[1]), null, 2)} as const;
export const PAYROLL_SOLUTIONS_INLINE_STYLES = ${JSON.stringify(
    headStyleMatches.map((match) => match[1]),
    null,
    2,
  )} as const;

export function PayrollSolutionsContent() {
  return h(
    Fragment,
    null,
      ${bodyCode}
