...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/solutions/hire/index.html");
const outputRoot = path.resolve(projectRoot, "public/solutions/hire");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/solutions/hire/", "/solutions/hire"],
  ["https://www.deel.com/solutions/hire", "/solutions/hire"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `solutions/hire/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/solutions/hire/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(text) {
  return text.replace(/\/_next\/image\/\?url=([^"'`\s<>&]+(?:%[0-9A-Fa-f]{2}[^"'`\s<>&]*)*)(?:&amp;|&)w=\d+(?:&amp;|&)q=\d+/g, (match, encodedUrl) => {
    try {
      ...
      return `/solutions/hire/${localRelativePath}`;
    } catch {
      return match;
    }
  });
...
async function main() {
  ...
  console.log(`Migrated Hire page to ${outputHtmlPath}`);
}
