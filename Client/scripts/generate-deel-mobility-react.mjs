...
const sourceHtmlPath = path.join(projectRoot, 'public', 'solutions', 'mobility', 'index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'deelMobility');
...
export const DEEL_MOBILITY_PAGE_TITLE = ${JSON.stringify(decodeHtml(title))};
export const DEEL_MOBILITY_HTML_CLASSES = ${JSON.stringify(htmlClasses)};
export const DEEL_MOBILITY_ROUTE_SLUG = "/solutions/mobility";
export const DEEL_MOBILITY_STYLESHEET_HREFS = ${JSON.stringify(stylesheetHrefs, null, 2)} as const;
export const DEEL_MOBILITY_INLINE_STYLES = ${JSON.stringify(inlineStyles, null, 2)} as const;

export function DeelMobilityContent() {
  return (
    h(Fragment, null,
    ${bodyCode}
  ));
