...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/solutions/it/index.html");
const outputRoot = path.resolve(projectRoot, "public/solutions/it");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/solutions/it/", "/solutions/it"],
  ["https://www.deel.com/solutions/it", "/solutions/it"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `solutions/it/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/solutions/it/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(text) {
  return text.replace(/\/_next\/image\/\?url=([^"'`\s<>&]+(?:%[0-9A-Fa-f]{2}[^"'`\s<>&]*)*)(?:&amp;|&)w=\d+(?:&amp;|&)q=\d+/g, (match, encodedUrl) => {
    try {
      ...
      return `/solutions/it/${localRelativePath}`;
    } catch {
      return match;
    }
  });
...
async function main() {
  ...
  console.log(`Migrated IT page to ${outputHtmlPath}`);
}
