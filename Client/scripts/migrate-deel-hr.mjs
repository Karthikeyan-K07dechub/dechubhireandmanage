...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/solutions/hr/index.html");
const outputRoot = path.resolve(projectRoot, "public/solutions/hr");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/solutions/hr/", "/solutions/hr"],
  ["https://www.deel.com/solutions/hr", "/solutions/hr"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `solutions/hr/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/solutions/hr/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(text) {
  return text.replace(/\/_next\/image\/\?url=([^"'`\s<>&]+(?:%[0-9A-Fa-f]{2}[^"'`\s<>&]*)*)(?:&amp;|&)w=\d+(?:&amp;|&)q=\d+/g, (match, encodedUrl) => {
    try {
      ...
      return `/solutions/hr/${localRelativePath}`;
    } catch {
      return match;
    }
  });
