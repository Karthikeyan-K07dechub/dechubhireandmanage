...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/solutions/payroll/index.html");
const outputRoot = path.resolve(projectRoot, "public/solutions/payroll");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/solutions/payroll/", "/solutions/payroll"],
  ["https://www.deel.com/solutions/payroll", "/solutions/payroll"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `solutions/payroll/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/solutions/payroll/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(html) {
  return html.replace(/\/_next\/image\/\?url=([^"'&]+)([^"'<>]*)/gi, (_, encodedUrl) => {
    try {
      ...
      return `/solutions/payroll/${localRelativePath}`;
    } catch {
      return "#";
    }
  });
...
async function collectLocalAssets() {
  ...
  async function walk(relativeDir) {
    ...
    for (const entry of entries) {
      ...
      assets.push({
        localPath: `solutions/payroll/${childRelativePath}`,
        bytes: stats.size,
      });
    }
  }
