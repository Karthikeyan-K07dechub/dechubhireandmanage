...
const sourceHtmlPath = path.join(projectRoot, 'public', 'solutions', 'hire', 'index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'deelHire');
...
async function main() {
  ...
  const title = extractSection(/<title>([\s\S]*?)<\/title>/i, html, 'Hire Solutions | Deel');
...
export const DEEL_HIRE_PAGE_TITLE = ${JSON.stringify(decodeHtml(title))};
export const DEEL_HIRE_HTML_CLASSES = ${JSON.stringify(htmlClasses)};
export const DEEL_HIRE_ROUTE_SLUG = "/solutions/hire";
export const DEEL_HIRE_STYLESHEET_HREFS = ${JSON.stringify(stylesheetHrefs, null, 2)} as const;
export const DEEL_HIRE_INLINE_STYLES = ${JSON.stringify(inlineStyles, null, 2)} as const;

export function DeelHireContent() {
  return (
    h(Fragment, null,
    ${bodyCode}
  ));
