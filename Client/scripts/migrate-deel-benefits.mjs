...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/solutions/benefits/index.html");
const outputRoot = path.resolve(projectRoot, "public/solutions/benefits");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/solutions/benefits/", "/solutions/benefits"],
  ["https://www.deel.com/solutions/benefits", "/solutions/benefits"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `solutions/benefits/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/solutions/benefits/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(text) {
  return text.replace(/\/_next\/image\/\?url=([^"'`\s<>&]+(?:%[0-9A-Fa-f]{2}[^"'`\s<>&]*)*)(?:&amp;|&)w=\d+(?:&amp;|&)q=\d+/g, (match, encodedUrl) => {
    try {
      ...
      return `/solutions/benefits/${localRelativePath}`;
    } catch {
      return match;
    }
  });
