...
const sourceHtmlPath = path.resolve(projectRoot, "../New_Page/solutions/mobility/index.html");
const outputRoot = path.resolve(projectRoot, "public/solutions/mobility");
...
const routeMappings = new Map([
  ["https://www.deel.com/", "/"],
  ["https://www.deel.com", "/"],
  ["https://www.deel.com/solutions/mobility/", "/solutions/mobility"],
  ["https://www.deel.com/solutions/mobility", "/solutions/mobility"],
...
async function downloadAsset(rawUrl) {
  ...
  downloadedAssets.push({
    url: rawUrl,
    localPath: `solutions/mobility/${localRelativePath}`,
...
function replaceMappedUrls(text) {
  ...
  sortedEntries.forEach(([remoteUrl, localRelativePath]) => {
    const localPath = `/solutions/mobility/${localRelativePath}`;
...
function rewriteNextImageOptimizerUrls(text) {
  return text.replace(/\/_next\/image\/\?url=([^"'`\s<>&]+(?:%[0-9A-Fa-f]{2}[^"'`\s<>&]*)*)(?:&amp;|&)w=\d+(?:&amp;|&)q=\d+/g, (match, encodedUrl) => {
    try {
      ...
      return `/solutions/mobility/${localRelativePath}`;
    } catch {
      return match;
    }
  });
...
async function main() {
  ...
  console.log(`Migrated HR page to ${outputHtmlPath}`);
}
