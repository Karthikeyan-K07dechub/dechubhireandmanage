...
const sourceHtmlPath = path.join(projectRoot, 'public', 'solutions', 'hr', 'index.html');
const outputDir = path.join(projectRoot, 'src', 'pages', 'deelHr');
...
async function main() {
  ...
  const title = extractSection(/<title>([\s\S]*?)<\/title>/i, html, 'Deel HR | Deel');
...
export const DEEL_HR_PAGE_TITLE = ${JSON.stringify(decodeHtml(title))};
export const DEEL_HR_HTML_CLASSES = ${JSON.stringify(htmlClasses)};
export const DEEL_HR_ROUTE_SLUG = "/solutions/hr";
export const DEEL_HR_STYLESHEET_HREFS = ${JSON.stringify(stylesheetHrefs, null, 2)} as const;
export const DEEL_HR_INLINE_STYLES = ${JSON.stringify(inlineStyles, null, 2)} as const;

export function DeelHrContent() {
  return (
    h(Fragment, null,
    ${bodyCode}
  ));
